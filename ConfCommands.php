...
class ConfCommands extends DrushCommands implements SiteAliasManagerAwareInterface {
    ...
    protected function runExport(SiteAlias $alias) {
        ...
        $process = Drush::drush($alias, 'cc', ['bin', 'config']);
        ...
        // If structure sync module exists, export blocks, menus, and taxonomies.
        if ($this->checkEnabledModule($alias, 'structure_sync')) {
            $this->write('Exporting blocks.');
            $process = Drush::drush($alias, 'export-blocks', []);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
            ...
            $process = Drush::drush($alias, 'export-taxonomies', []);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
            ...
            $this->write('Exporting menus.');
            $process = Drush::drush($alias, 'export-menus', []);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
                $this->write('Exported menus.', 'success', TRUE);
            }
            else {
                $this->write('Failure exporting menus.', 'error', TRUE);
                $this->write($process->getErrorOutput());
                return new CommandError();
            }
        }
        ...
        $skip = "search_api_task,crop,file,media,user,webform_submission";

        // If default content deploy module exists, export default content.
        if ($this->checkEnabledModule($alias, 'default_content_deploy')) {
            $this->io()->title('Exporting content');
            $process = Drush::drush($alias, 'default-content-deploy:export-site', ['--skip_entity_type=' . $skip]);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
                $this->write('Exported content.', 'success', TRUE);
            }
            else {
                $this->write('Failure exporting content.', 'error', TRUE);
                $this->write($process->getErrorOutput());
                return new CommandError();
            }
        }
    ...
    protected function runImport(SiteAlias $alias) {
        ...
        if ($this->checkEnabledModule($alias, 'structure_sync')) {
            ...
            $this->write('Importing menus.');
            $process = Drush::drush($alias, 'import-menus', [], ['choice' => 'full']);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
                $this->write('Imported menus.', 'success', TRUE);
            }
            else {
                $this->write('Failure importing menus.', 'error', TRUE);
                $this->write($process->getErrorOutput());
                return new CommandError();
            }
        }
        ...
        if ($this->checkEnabledModule($alias, 'default_content_deploy')) {
            ...
            $process = Drush::drush($alias, 'default-content-deploy:import', []);
            $success = ($this->io()->isVerbose()) ? $process->run($process->showRealtime(), $alias->get('envs')) : $process->run(null, $alias->get('envs'));
            if ($success === 0) {
